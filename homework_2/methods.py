...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        if self._method == 'Constant':
            ...
            a = previous_alpha if previous_alpha is not None else self.alpha_0
            while oracle.func_directional(x_k, d_k, a) > oracle.func(x_k) + self.c1 * a * oracle.grad_directional(x_k, d_k, 0):
                a = a / 2
            return a
        elif self._method == 'Wolfe':
            phi = lambda x: oracle.func_directional(x_k, d_k, x)
            derphi = lambda x: oracle.grad_directional(x_k, d_k, x)
            a, *_ = scalar_search_wolfe2(phi, derphi, c1 = self.c1, c2 = self.c2)
            return a
...
class GradientDescent(object):
    ...
    def run(self, max_iter=100):
        ...
        """
        st = time.time()
        grad_norm_x0 = np.linalg.norm(self.oracle.grad(self.x_0))**2
        x = self.x_0.copy()
        a = None
        for i in range(max_iter):
            f = self.oracle.func(x)
            grad = -self.oracle.grad(x)
            grad_norm  = np.linalg.norm(grad)

            self.hist['time'].append(time.time() - st)
            self.hist['func'].append(f)
            self.hist['grad_norm'].append(grad_norm)
            if len(x) <= 2:
                self.hist['x'].append(x.copy())
            self.hist['x_star'] = x.copy()

            if grad_norm_x0 == 0 or grad_norm**2 / grad_norm_x0 <= self.tolerance:
                break

            a = self.line_search_tool.line_search(self.oracle, x, grad, a)
            if a is None:
                break
            x = x + a * grad
        f = self.oracle.func(x)
